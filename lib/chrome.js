...
app.page = {
  ...
  "sendToTab": function (tabId, options, frameId, attempt) {
    attempt = attempt || 0;
    var sendOptions = frameId !== null && frameId !== undefined ? {"frameId": frameId} : undefined;
    app.log("[DarkMode] background sending to tab", {tabId: tabId, method: options.method, attempt: attempt, frameId: frameId});
    chrome.tabs.sendMessage(tabId, options, sendOptions, function (response) {
      if (chrome.runtime.lastError) {
        app.warn("[DarkMode] background sendMessage failed", chrome.runtime.lastError.message, options, tabId, attempt);
        if (attempt < 2) {
          self.setTimeout(function () {
            app.page.sendToTab(tabId, options, frameId, attempt + 1);
          }, 250);
        }
      }
    });
  ...
  "send": function (id, data, tabId, frameId) {
    ...
    if (tabId !== null && tabId !== undefined) {
      app.page.sendToTab(tabId, options, frameId, 0);
      return;
    }
    chrome.tabs.query({}, function (tabs) {
      if (tabs && tabs.length) {
        tabs.forEach(function (tab) {
          if (tab) {
            app.page.sendToTab(tab.id, options, frameId, 0);
          }
        });
      }
    });
...
app.button = {
  "title": function (title, callback) {
    chrome.action.setTitle({"title": title}, function (e) {
      if (callback) callback(e);
    });
  },
  "on": {
  ...
  "icon": function (path, callback) {
    if (path && typeof path === "object") {
      chrome.action.setIcon({"path": path}, function (e) {
        if (callback) callback(e);
      });
    } else {
...
app.storage = (function () {
  chrome.storage.onChanged.addListener(function () {
    chrome.storage.local.get(null, function (e) {
      app.storage.local = e;
      app.enableLog = app.storage.local.enable_log === true;
...
// Ensure background has the latest saved storage values when the service worker starts.
app.storage.load(function () {
  // No-op callback; storage is now cached and ready for page load messages.
});