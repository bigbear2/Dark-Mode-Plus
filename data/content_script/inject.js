var debug = {
  ...
  update: function (storage) {
    debug.enabled = storage && storage.enable_log === true;
  },
  log: function () {
...
var background = (function () {
  ...
  chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    debug.log("[DarkMode] content received background message", request, sender);
    for (var id in tmp) {
  ...
  return {
    ...
    "send": function (id, data) {
      debug.log("[DarkMode] content sending page-to-background", id, data);
...
var config = {
  ...
  "apply": {
    "style": function (e) {
      ...
      debug.log("[DarkMode] content apply style", {loc: loc, href_g: href_g, href_c: href_c, text_c_len: text_c ? text_c.length : 0, text_n_len: text_n ? text_n.length : 0, options: options});
      /*  */
  ...
  "normalizeStorage": function (storage, url) {
    var tmp = storage || {};
    if (!tmp.whitelist) tmp.whitelist = [];
    if (!tmp.cookie) tmp.cookie = [];
    if (tmp.nativeignore === undefined) tmp.nativeignore = false;
    if (tmp.nativeforceborder === undefined) tmp.nativeforceborder = true;
    if (tmp.native === undefined) tmp.native = website.custom.native.css.replace(/        /g, '');
    if (tmp.state === undefined) tmp.state = "light";
    if (tmp.site_scope === undefined) tmp.site_scope = "host";
    if (!Array.isArray(tmp.whitelist)) tmp.whitelist = [];
    var hasTheme = false;
    ...
    if (!hasTheme) tmp["dark_" + website.total.themes.number] = true;
    tmp.site_state = config.preference.site.state(tmp, url);
    return tmp;
  },
  "load": function () {
    debug.log("[DarkMode] content load start", document.location.href);
    chrome.storage.local.get(null, function (storage) {
      debug.update(storage);
      debug.log("[DarkMode] content local storage loaded", storage);
      config.update({
        "top": document.location.href,
        "uri": document.location.href,
        "storage": config.normalizeStorage(storage, document.location.href),
        "frameId": 0,
        "hostname": config.hostname(document.location.href),
        "reload": false
      });
      debug.log("[DarkMode] content applied local fallback update");
    });
    background.send("load");
    debug.log("[DarkMode] content sent load request to background");
  ...
  "update": function (e) {
    ...
    debug.log("[DarkMode] content update", {top: top, uri: uri, hostname: hostname, state: e.storage.state, site_state: e.storage.site_state, reload: e.reload, whitelist: e.storage.whitelist});
    /*  */
    if (darkness) {
    ...
    /* If no per-site preference is saved, do not apply the global theme (leave site unchanged) */
    if (e.storage.site_state === undefined) {
      return;
    } else {
      // apply per-site override
      e.storage.state = e.storage.site_state;
    }
    /*  */
    for (var i = 1; i <= website.total.themes.number; i++) {
